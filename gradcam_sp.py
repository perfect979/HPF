...
class GradCAM2:
    ...
    def __call__(self, input, target=None):
        ...
        cam3 = CAM(activation3.squeeze().detach().cpu().numpy(), gradient3.squeeze().detach().cpu().numpy())
        cam2 = CAM(activation2.squeeze().detach().cpu().numpy(), gradient2.squeeze().detach().cpu().numpy())
        cam1 = CAM(activation1.squeeze().detach().cpu().numpy(), gradient1.squeeze().detach().cpu().numpy())

        cam1 = torch.from_numpy(cam1).cuda()
        cam2 = torch.from_numpy(cam2).cuda()
        cam3 = torch.from_numpy(cam3).cuda()
        cam23 = torch.max(cam2, cam3).unsqueeze(0).unsqueeze(-1)
        ...
        interpolated_cam = torch.sum(index_points(cam23, idx) * dists_weight.view(B1, N1, 3, 1), dim=2).squeeze()
        interpolated_cam = torch.from_numpy(std(interpolated_cam.squeeze().detach().cpu().numpy())).cuda()
        cam123 = torch.max(cam1, interpolated_cam).unsqueeze(0).unsqueeze(-1)
        ...
        norm = torch.sum(dist_recip, dim=2, keepdim=True)
        dists_weight = dist_recip / norm
        interpolated_cam = torch.sum(index_points(cam123, idx) * dists_weight.view(B0, N0, 3, 1), dim=2).squeeze()
        cam = std(interpolated_cam.squeeze().detach().cpu().numpy())
