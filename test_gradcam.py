...
class GradCAM2:
    ...
    def __call__(self, input, target=None):
        ...
        # activation1 = torch.cat((torch.max(activation1, dim=3).values, torch.max(activation1_1, dim=3).values, torch.max(activation1_2, dim=3).values), dim=1)  # [1, 320, 512]
        # gradient1 = torch.cat((torch.max(gradient1, dim=3).values, torch.max(gradient1_1, dim=3).values, torch.max(gradient1_2, dim=3).values), dim=1)  # [1, 320, 512]

        # activation2 = torch.cat((torch.max(activation2, dim=3).values, torch.max(activation2_1, dim=3).values, torch.max(activation2_2, dim=3).values), dim=1)  # [1, 640, 128]
        # gradient2 = torch.cat((torch.max(gradient2, dim=3).values, torch.max(gradient2_1, dim=3).values, torch.max(gradient2_2, dim=3).values), dim=1)  # [1, 640, 128]

        # activation1 = torch.cat((torch.sum(activation1, dim=3), torch.sum(activation1_1, dim=3), torch.sum(activation1_2, dim=3)), dim=1)  # [1, 320, 512]
        # gradient1 = torch.cat((torch.sum(gradient1, dim=3), torch.sum(gradient1_1, dim=3), torch.sum(gradient1_2, dim=3)), dim=1)  # [1, 320, 512]

        # activation2 = torch.cat((torch.sum(activation2, dim=3), torch.sum(activation2_1, dim=3), torch.sum(activation2_2, dim=3)), dim=1)  # [1, 640, 128]
        # gradient2 = torch.cat((torch.sum(gradient2, dim=3), torch.sum(gradient2_1, dim=3), torch.sum(gradient2_2, dim=3)), dim=1)  # [1, 640, 128]
        ...
        activation3 = activation3.squeeze(-2)
        gradient3 = gradient3.squeeze(-2)

        ################将[1,1664,128]拼接到[1, 128, 32, 512]

        dists = square_distance(xyz0, xyz2)  # 2048, 128
        ...
        interpolated_points = torch.sum(index_points(activation3, idx) * dists_weight.view(B1, N0, 3, 1), dim=2)
        interpolated_grads = torch.sum(index_points(gradient3, idx) * dists_weight.view(B1, N0, 3, 1), dim=2)
        ...
        activations = interpolated_points.permute(0, 2, 1).squeeze().detach().cpu().numpy()  # [1408, 2048]
        gradients = interpolated_grads.permute(0, 2, 1).squeeze().detach().cpu().numpy()
